...
import {
  ...
  deletarInsumo,
  IInsumoResponse,
} from '@/app/frontend/use-cases/InsumoCases'

export default function CadastroDeInsumoForm({
  idInsumo,
}: {
  idInsumo?: string
}) {
  ...
  useEffect(() => {
    if (!idInsumo || !idPropriedadeSelecionada) return

    buscarInsumoPorId(idInsumo, idPropriedadeSelecionada).then((data) => {
      const insumo = data?.data.dataConnection
      if (!insumo) return
      setNome(insumo.nome ?? '')
      setQuantidade(Number(insumo.quantidade ?? 1))
      setCusto(Number(insumo.custo ?? 0))
      setUnidadeMedida(insumo.unidade_medida ?? '')
    })
  }, [idInsumo, idPropriedadeSelecionada])
  ...
  async function salvar(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      let resultado: IInsumoResponse | undefined
      if (idInsumo) {
      ...
      if (resultado && resultado.success) {
        setToast({
          title: idInsumo ? 'Insumo atualizado!' : 'Insumo criado!',
          description: `${nome} foi ${
            idInsumo ? 'atualizado' : 'cadastrado'
          } com sucesso.`,
          variant: 'success',
        })
  ...
  async function resultadoDelecao() {
    ...
    try {
      const resultado = await deletarInsumo(idInsumo)
      if (resultado && resultado.success) {
        setToast({
  ...
  return (
    <>
      ...
      <DialogConfirmaDelecao
        ...
        temCerteza="Tem certeza que deseja excluir o insumo?"
